...
def run_connector_scan(
    *,
    ...
    payload = {
        ...
        "metadata": metadata or {"source": "sgraph_trusted_decrypt_flow"},
    }
    return client.scan_attachment(payload)


def enforce_connector_verdict(scan_result: dict[str, Any]) -> str:
    verdict = str(scan_result.get("verdict", "quarantine")).strip().lower()
    if verdict == "allow":
        return "allow"
    if verdict == "block":
        raise RuntimeError("blocked by connector security verdict")
    return "quarantine"
